...
export default function Home() {
  ...
  const fetchBooks = async (pageNum = 1, refresh = false) => {
    try {
      ...
      const combinedBooks = refresh || pageNum === 1
        ? data.books
        : [...books, ...data.books];

      const uniqueBooks = Array.from(
        new Map(combinedBooks.map(book => [book._id, book]))
      ).map(([, book]) => book);
      ...
      <FlatList
        ...
        ListHeaderComponent={
          <View style={styles.header}>
            <Text style={styles.headerTitle}>BookEater🦈</Text>
            <Text style={styles.headerSubtitle}>Discover great reads from the community</Text>
          </View>
        }

        ListFooterComponent={
          hasMore && Array.isArray(books) && books.length > 0 ? (
